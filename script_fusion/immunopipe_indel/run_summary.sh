...
VCF_file=VCF_combined
cat ${OUTPUT_DIR}/VCF_frameshift ${OUTPUT_DIR}/VCF_nonframeshift > ${OUTPUT_DIR}/$VCF_file

echo '>>>>> run summary_MHCI.py/summary_MHCII.py <<<<<'
echo $hlaAllele | awk -v vcf=$OUTPUT_DIR/$VCF_file -v mutPep=$OUTPUT_DIR/${PID}_mut.fa -v refPep=$OUTPUT_DIR/${PID}_ref.fa -v sumI=$PIPELINE_DIR/summary_MHCI_indel.py -v sumII=$PIPELINE_DIR/summary_MHCII_indel.py '{for (i=1;i<=NF;i++) {if (index($i,"HLA-")!=0) print "python", sumI, $i"_mut_reformat", $i"_ref_reformat", vcf, mutPep, refPep, $i"_summary"; else  print "python", sumII, $i"_mut_reformat", $i"_ref_reformat", vcf, mutPep, refPep, $i"_summary" } }' > $OUTPUT_DIR/intermediate.sh
...
# filter out neoepitope-peptited which are identical to the reference peptide 
